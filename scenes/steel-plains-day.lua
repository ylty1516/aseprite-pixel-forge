-- 钢铁巨构平原：王国（Kingdom: Two Crowns）画风 —— 剪影 + 渐变天空 + 暖光点
-- 画法：景物一律单色剪影（flat），深度靠三级暗度 + 大气透视；细节由观者想象补全；
--       窗灯是唯一的暖色点。渲染：forge scene scenes/steel-plains-day.lua --out <目录> --frames 8 --gif
return {
  ...
  layers = {
    ...
    { type = "clouds", count = 3, y0 = 30, y1 = 76, ramp = "sky", density = 0.22,
      wmin = 70, wmax = 150, hmin = 6, hmax = 12 },
    ...
    { type = "viaduct", x0 = -24, x1 = 664, y = 128, deck_h = 5, base = 200,
      broken = true, broken_count = 3, debris = false,
      ramp = "shadow", level = 3, pier_min = 52, pier_max = 96 },
    { type = "megastructure", y = 178, hmin = 96, hmax = 150,
      ...
      styles = { "frame" }, flat = true },
    -- 远层水彩式洗入天空
    { type = "grade", spec = {
      ["shadow"] = { target = "sky", blend = 0.55, shift = 1 },
    } },
    { type = "fog", ramp = "sky", strength = 0.4, baseLevel = 3, levelSpan = 1,
      field = function(_, y)
        local d = (y - 140) / 60
        return math.max(0, 1 - d * d)
      end },
    ...
    { type = "megastructure", x0 = 348, x1 = 392, y = 205,
      ...
      ramp = "shadow", level = 2, styles = { "frame" }, flat = true, glow = true },
    ...
    { type = "megastructure", x0 = 572, x1 = 660, y = 205,
      ...
      ramp = "shadow", level = 2, styles = { "steelarch" }, flat = true, glow = true },
    { type = "fog", ramp = "sky", strength = 0.22, baseLevel = 3, levelSpan = 1,
      field = function(_, y)
        local d = (y - 186) / 36
        return math.max(0, 1 - d * d)
      end },
    ...
    { type = "fog", ramp = "sky", strength = 0.14, baseLevel = 3, levelSpan = 1,
      field = function(_, y)
        local d = (y - 214) / 44
        return math.max(0, 1 - d * d)
      end },
    ...
    { type = "megastructure", x0 = 462, x1 = 502, y = 248,
      ...
      ramp = "shadow", level = 1, styles = { "mast" }, flat = true, glow = true },
